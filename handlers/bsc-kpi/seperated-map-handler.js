"use strict"
/**
 * Bộ xử lý phân rã bản đồ chiến lược cho bộ phận
 * 
 * ver 1.0 
 * Created by cuong.dq
 * date: 23/08/2019
...
class Handler {
     ...
     * @param {*} next 
     */
    async createSeperatedMapKpi(req,res,next){
        ...
        if (dataJson.id<0) { 
            ...
            await db.insert(arrObj.convertSqlFromJson("seperated_map_kpi", dataJson, []));
            //console.log(rt);
            ...
            //cần update
            await db.update(arrObj.convertSqlFromJson("seperated_map_kpi", dataJson, ["id"]));
