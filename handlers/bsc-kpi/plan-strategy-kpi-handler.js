"use strict"
/**
 * Bộ xử lý lập kế hoạch kpi cho công ty/trung tâm
 * 
 * Nhập kế hoạch,
...
const updateParentDepth = (reportId, organizationId, depth) => {
        ...
        try {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (el.parent_id && el.total_effective) {
                    let updateResult = {
                        report_id: reportId,
                        id: el.parent_id, // cập nhập lên cấp ca của nó Tỷ lệ hoàn thành tổng
                        operator_method: '', // canh va goc ko co toan tu
                        total_effective: el.total_effective,
                        // , điểm quy đổi này không lấy ra ở đây
                    ...
                    await db.update(arrObj.convertSqlFromJson('report_map_kpi', updateResult, ['report_id', 'id']));
                }
            }
        } catch (e) {
            console.log('Loi select & update', e);
        } finally {
            resolve('finish!')
        }
    })
        ...
        .then(els => {
            els.forEach(async el => {
                if (el.parent_id && el.total_effective) {
                    ...
                    // cap nhap cap cao hon 1 cap
                    await updateParentDepth(reportId, organizationId, el.depth - 1);
                    //
                }
            })
...
const updateResultEffectiveId = (reportId, id) => {
    db.getRst(`select a.* from report_map_kpi a
 ...
 * @param {*} organizationId 
 */
const updateWeightPercentReportStrategyMap = (reportId, organizationId) => {
            ...
            .then(results => {
                ...
                treeOrder.forEach(async (el) => {
                    let jsonForUpdate = {
                        ...
                        is_leaf: el.$is_leaf, //gán lá cây để tổng hợp báo cáo cho nhanh
                        weight_percent: el.$weight_percent,
                        parent_weight_percent: el.$parent_weight_percent,
                        root_weight_percent: el.$root_weight_percent
                    }
                    await db.update(arrObj.convertSqlFromJson('report_map_kpi', jsonForUpdate, ['id']))
                })
            })
...
class Handler {
     ...
     * @param {*} next 
     */
    async createPlanStrategyKpi(req, res, next) {
        ...
        if (// dataJson.organization_id&&
            ...
            //cần update
            await db.update(arrObj.convertSqlFromJson("report_map_kpi", dataJson, ["report_id", "id"]));
            ...
            if (dataJson.organization_id && (dataJson.status !== undefined || dataJson.weight !== undefined)) {
                ...
                updateWeightPercentReportStrategyMap(dataJson.report_id, dataJson.organization_id)
            }
     ...
     * @param {*} next 
     */
    createPlanStrategyMap(req, res, next) {
     ...
     * @param {*} next 
     */
    deletePlanStrategyMap(req, res, next) {

        if (req.paramS.organization_id && req.paramS.report_id) {
            db.delete(
                arrObj.convertSqlFromJson('report_map_kpi'
                    , {
     ...
     * @param {*} next 
     */
    getPlanStrategyMap(req, res, next) {

        db.getRsts(`select * from report_map_kpi a
                    where a.status=1 and a.weight>0
                    and a.organization_id = ${(req.paramS.organization_id ? req.paramS.organization_id : 0)}
                    and a.report_id = ${(req.paramS.report_id ? req.paramS.report_id : 0)}
                    order by order_1
                    `)
            .then(results => {
                // console.log(results);
