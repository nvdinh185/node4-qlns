 ...
 * 
 * ver 2.1 
 * Chỉnh sửa ngày 10/9/2019
 * Lỗi khi nâng cấp version node lên 12.10.0 
 * Error: write EPROTO khi sử dụng request một link https
 * Do đó, cần phải thêm tham số : node --tls-min-v1.0 local server
 * Khi thực hiện lệnh node để lỗi xác thực token qua proxy không bị lỗi
 * 
 * version 2.0
 * thêm lấy public user/friends/follows/contacts của user
 * repair 31/05/2019
 * 
 * ver 1.0
            ...
            if (aliveToken && verifyExpire(req)){
                ...
                proxy.post(authServer + '/ext-auth/authorize-token', { json: {token: req.token} } //du lieu parse tu postProcess
                    , (error, res, body) => {
                        if (error) {
                            reject(error);
                        }else if (body.status&&body.user_info) {
                            tokenSession.push({
...
/**
 * trả về req.publicUsers = danh sách public lấy được từ API server
 * 
 * @param {*} req 
 * @param {*} res 
 * @param {*} next 
...
const getPublicUser= (req, res, next) => {
    proxy.get(authServer + '/ext-auth/get-users-info', null //khong truyen du lieu gi 
        , (error, res, body) => {
            if (error){
                console.log('Lỗi request: ' + authServer + '/ext-auth/get-users-info', error)
            }else if (body) {
                try{
                    let result = JSON.parse(body);
                    if (result && result.status===1&&result.users){
                        req.publicUsers = result.users;
                    }
                }catch(e){}
            } 
            next();
...
module.exports = {
  ...
  //trả về danh sách public user đã post tin lên  
